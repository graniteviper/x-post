...
export default function Home() {
  ...
  const handleGenerate = async (input: string, tone: string) => {
    ...
    if (!session) {
      ...
      const response = await axios.post<IncrementPromptResponse>("/api/increment-prompt", {
        userId: session.user.id,
      });
  ...
  return (
    <AuroraBackground className="min-h-screen flex flex-col overflow-hidden px-4 sm:px-6 md:px-10">
      <Toaster
        toastOptions={{
          style: {
            background: "rgba(255, 255, 255, 0.95)",
      ...
      <div className="w-full fixed top-2 sm:top-4 left-0 z-50 bg-transparent">
        <Navbar anonymousPostsUsed={anonymousPostsUsed} />
      </div>

      <div className="flex flex-col items-center justify-center flex-1 pt-28 sm:pt-32 pb-20 space-y-8 sm:space-y-10">
        <motion.div
          animate={{
            y: result || isGenerating ? -40 : 0,
            scale: result || isGenerating ? 0.95 : 1,
          }}
          transition={{ duration: 0.7, ease: "easeInOut" }}
          className="w-full"
        >
          <MainBox onGenerate={handleGenerate} disableGenerate={disableGenerate} />
        </motion.div>
        ...
        {(isGenerating || result) && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1 }}
            transition={{ duration: 0.6, ease: "easeOut" }}
            className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-6 sm:p-10 md:p-12 flex flex-col items-center gap-6 shadow-xl w-full max-w-4xl"
          >
            {isGenerating ? (
              <>
                <LoaderOne />
                <p className="text-zinc-600 text-base sm:text-lg mt-2 text-center">
                  Generating your post...
                </p>
              </>
            ) : (
              <motion.p
                ...
                animate={{ opacity: 1 }}
                transition={{ delay: 0.3 }}
                className="text-zinc-600 text-lg sm:text-xl leading-relaxed text-center break-words px-2"
              >
                {result}
              </motion.p>
            )}
